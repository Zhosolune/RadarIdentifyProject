...
- 数据流：
  ...
  graph TD
      ...
      B -->|直接调用| C[SignalService]
      C -->|处理| D[SignalProcessor]
      D -->|返回| E[切片结果]
      E -->|回调处理| B
      B -->|发射信号| F[UI显示]
  ```
...
- 状态：✓ 已重构完成
- 功能：处理信号数据文件的导入和预处理
- 架构模式：Handler → Service（直接异步调用）
- 主要组件：
  - SignalImportHandler：处理UI事件和异步调用
  - SignalService.load_signal_file：执行实际的导入逻辑
- 调用流程：
  1. Handler接收UI事件
  2. 直接提交Service方法到线程池
  3. 异步处理导入逻辑
...
- 架构模式：Handler → Service（直接异步调用）
- 主要组件：
  - SignalSliceHandler：处理UI事件和异步调用
  - SignalService.start_slice_processing：执行实际的切片逻辑
- 调用流程：
  1. Handler接收UI事件
  2. 直接提交Service方法到线程池
  3. 异步处理切片逻辑
...
- 数据流：
    ```mermaid
    graph TD
        A[Handler接收事件] -->|直接调用| B[Service处理]
        B -->|异步执行| C[切片处理]
        C -->|回调处理| D[结果返回]
        D -->|信号发射| E[UI更新]
    ```
...
##### 3.1.4.3 信号切片功能 (signal_slice_handler.py, signal_service.py)
- 状态：✓ 已重构完成
- 功能：实现信号数据的切片处理和切片导航功能

- 实现文件：
  1. SignalSliceHandler：切片UI处理器
     - 功能：处理切片相关的UI事件和异步调用
     - 架构：直接调用Service，消除Task抽象层
     - 主要方法：
       - start_slice：启动切片处理（直接异步调用Service）
       - request_next_slice：请求下一个切片数据
       - _handle_slice_result：处理切片结果回调
  ...
  3. 架构优化：
     - 消除了SignalSliceTask中间层
     - Handler直接调用Service方法
     - 保持异步执行和线程安全
     - 统一了调用模式
